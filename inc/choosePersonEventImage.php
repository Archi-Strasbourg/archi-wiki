...
echo "<h2>Sélection de l'image principale pour ".
    $person->prenom.' '.$person->nom." sur l'événement ".
    $event->getTitle().'</h2>';


$dummyImage = new ArchiImage($row['idImage']);
$reqImages = $dummyImage->getImagesFromEvenement(
    array('select'=>'hi1.idImage', 'idEvenement'=>$_GET['idEvent'])
);
...
while ($row = mysql_fetch_assoc($res)) {
    ...
    echo '<li><img src="getPhotoSquare.php?id='.
        $image->getIdHistoriqueImage().'" /></li>';
}
echo '</ul>';
