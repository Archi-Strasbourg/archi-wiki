...
*/
    ...
    public function ajouter()
    {
        ...
        } elseif (isset($this->variablesPost['ajoutSousEvenement'])) {
            $tabForm=$this->getEvenementFields('nouveauDossier');
    ...
    public function modifier($id)
    {
        ...
        } elseif (isset($this->variablesPost['evenementSimple'])) { // evenementSimple correspond au nom du bouton submit du formulaire, on enregistre les modifications
        // modification de l'evnement
            ...
            if (count($erreur) == 0) {
                ...
                // la modif des personnes est authorisee seulement pour les moderateurs et les admins
                    ...
                    } else {   // de toute facon on nettoie les valeurs si le champ est vide
                        $sqlNettoyePersonne = 'DELETE FROM _evenementPersonne WHERE idEvenement='.$idEvenement;
                        $this->connexionBdd->requete($sqlNettoyePersonne);
                    }
                }
    ...
    public function afficher($idEvenement = null, $modeAffichage = '', $idHistoriqueEvenement = null, $paramChampCache = array(), $params = array())
    {
        ...
        if (isset($sqlWhere) && mysql_num_rows($rep) > 0) {
            ...
            if ($res->groupe!=3) {
                ...
                $source = "";
                if ($res->idSource!=0) {
                    $source="Source : <a href='".$this->creerUrl(
                        '',
                        'listeAdressesFromSource',
                        array(
                                    'source'=>$res->idSource,
                                    'submit'=>'Rechercher'
									...
									document.getElementById('iframe').src='".$this->creerUrl(
                        '',
                        'descriptionSource',
                        array(
    ...
    public function getIdEvenementsFromRecherche($criteres = array(), $templateName = 'listeEvenement.tpl', $modeAffichage = '')
    {
        ...
        if (!isset($criteres['ordre'])) {
            ...
            switch ($criteres['ordre']) {
                ...
                case 'type':
                    ...
                    break;
                case 'dateCreation':
                    $sqlOrdre = 'hE.dateCreationEvenement';
                    break;
                default:
                    $sqlOrdre = 'hE.titre';
            }
        ...
        if (isset($criteres['selection']) and is_numeric($criteres['id'])) {
            switch ($criteres['selection']) {
                case 'typeStructure':
                    $tabSqlWhere[] = 'hE.idTypeStructure='.$criteres['id'];
                    break;
                case 'typeEvenement':
                    $tabSqlWhere[] = 'hE.idTypeEvenement='.$criteres['id'];
                    break;
                case 'source':
                    $tabSqlWhere[] = 'hE.idSource='.$criteres['id'];
                    break;
                case 'utilisateurAjout':
                    $tabSqlWhere[] = 'hE.idUtilisateur='.$criteres['id'];
                    break;
                case 'personne':
                    $tabSqlWhere[] = '_eP.idPersonne='.$criteres['id'];
                    $tabSqlJoin[]  = 'LEFT JOIN _evenementPersonne _eP USING (idEvenement) ';
                    break;
                case 'courant':
                    $tabSqlWhere[] = '_eCA.idCourantArchitectural='.$criteres['id'];
                    $tabSqlJoin[]  = 'LEFT JOIN _evenementCourantArchitectural _eCA USING (idEvenement) ';
                    break;
            }
        }
    ...
    public function getAddressText($arrayAddresses)
    {
        ...
        if (empty($arrayAddresses [0] ['nomRue']) && $arrayAddresses [0] ['numero'] == 0) {
            if (! empty($arrayAddresses [0] ['nomQuartier'])) {
                if (! empty($arrayAddresses [0] ['nomSousQuartier'])) {
                    $returnText .= $arrayAddresses [0] ['nomSousQuartier']." / ";
                }
                $returnText .= $arrayAddresses [0] ['nomQuartier'];
            }
        }
        return $returnText;
    }
