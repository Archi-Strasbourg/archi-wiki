...
class archiRecherche extends config {
	...
	*/
	 ...
	 */
	private function getInfoAdresses($sqlWhere = '',$params =array()){
		$limit = "";
		$order ="DESC ";
		$returnField ="idHistoriqueAdresse";
	...
	public function searchByCriterias($criterias = array()){
		...
		if(!empty($criterias)){
			...
			if(isset($criterias['afficheResultatsSurCarte']) && $criterias['afficheResultatsSurCarte']){
				...
				$idHistoriqueEvenementArray = $this->getInfoAdresses($sqlWhere,$criterias); //Execute the fulltext research with the Where clause generated by
				//$idHistoriqueEvenementArray = $this->getInfoAdresses($sqlWhere,array_merge($criterias,array('returnField'=>'idAdresse'))); //Execute the fulltext research with the Where clause generated by
				$nbResult = count($this->getInfoAdresses($sqlWhere,$criterias)); //Count the nb of result
			...
			//Affichage par liste
				...
				$sqlWhere = $this->buildWhereClause($criterias); // Generating the where clause with the criterias
				$idHistoriqueEvenementArray = $this->getInfoAdresses($sqlWhere,$criterias); //Execute the fulltext research with the Where clause generated by
				$nbResult = count($this->getInfoAdresses($sqlWhere,array('motcle' => $motcle))); //Count the nb of result
				$a = new archiAdresse();
