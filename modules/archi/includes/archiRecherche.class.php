	...
	*/
	...
	private function getInfoAdresses($sqlWhere = '',$params =array()){
		...
		$requestTestMatch ="
				...
				(MATCH (nomQuartier) AGAINST ('".$params['motcle']."' IN BOOLEAN MODE)) as matchQuartier,
	...
	public function searchByCriterias($criterias = array()){
		...
		if(!empty($criterias)){
			...
			//Affichage par liste
				...
				/*
				debug(array(
						'delta t1 t2 ; build where clause' => ($t2-$t1),
						'delta t2 t3 ; getInfoAdresse' => ($t3-$t2),
						'delta t3 t4 ; affectation' => ($t4-$t3),
						'delta t4 t5 ; affectation' => ($t5-$t4),
						'delta t5 t6 ; nouvel objet archi' => ($t6-$t5),
						'delta t6 t7 ; displayList ' => ($t7-$t6)
				));
				*/
			}
		}
