...
class tableau
{
	...
	// construction d'un tableau html ou le pr�cise le nom de colonnes en parametres
	public function createHtmlTableFromArray($nbColonnes=4,$styleTableau='border:1px;',$classTableau='',$cellProperty='',$tableauProperties='')
	{
		...
		$ligne=0; // coordonn�es horizontales
		$colonne=0; // coordonn�es verticales
		for($i=0 ; $i<$nbCases ; $i++)
		{
			if($colonne==0)
			{
	...
	// fonction permettant d'afficher un tableau de photo par exemple avec un commentaire qui sera align� dans la ligne suivante du tableau
	...
	// creation d'un tableau a base de divs , permettant ainsi d'avoir des cases aux formats diff�rents
	// attention : si problemes avec ie , mettre un <div style='clear:left;'></div> a la fin dans le conteneur
	//
	// si nbColonnes est pr�cis� dans les parametres, on met un div autour des div afin d'aligner les cellules
